const RECAPTCHA_SITE_KEY = "6LebReMsAAAAAEnwESSzeOvEjcfnjB8WdQ-S5UUy";
...
function renderForumPosts() {
  ...
  if (!posts.length) {
    list.innerHTML = '<article class="forum-item"><strong>No questions yet.</strong><p>Be the first to ask about Wrestling, Taekwondo, Judo, or Volleyball.</p></article>';
    return;
  }
...
function renderBlogBoardPosts() {
  ...
  if (!posts.length) {
    list.innerHTML = '<article class="forum-item"><strong>No communication posts yet.</strong><p>Coaches, trainers, students, and parents can share their first update here.</p></article>';
    return;
  }
...
const SPORT_VISUALS = [
  ...
  },
  {
    ...
    alt: "A team lifting one another over an obstacle"
  },
  {
    key: "coaching-model",
    label: "Coaching Model",
    keywords: ["coaching model", "coaches", "coach", "schedules", "player profiles", "player development"],
  ...
  },
  {
    ...
    alt: "Athletes running on a track, representing academy updates"
  },
  {
    key: "football",
    label: "Football",
  ...
  },
  {
    key: "ballet",
    label: "Ballet",
    keywords: ["ballet", "dance", "movement"],
  ...
  },
  {
    key: "events",
    label: "Events",
    keywords: ["event", "events", "competition", "fixture"],
    src: "https://commons.wikimedia.org/wiki/Special:FilePath/Athletics%20track%20running.jpg",
    alt: "Athletes running during a track event"
  },
  {
    key: "training",
    label: "Training",
    keywords: ["training", "community", "coaching", "session"],
...
const BACKGROUND_VIDEO = {
  poster: "https://commons.wikimedia.org/wiki/Special:FilePath/Volleyball%20Match%20%282666331886%29.jpg",
  src: "https://cdn.coverr.co/videos/coverr-girls-playing-volleyball-1566140199457?download=1080p"
};

function createFallbackVisual(label, key) {
  const safeLabel = String(label || "SRC Sports").replace(/[<>&"']/g, "");
  const hue = Math.abs(String(key || safeLabel).split("").reduce((total, char) => total + char.charCodeAt(0), 0)) % 360;
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 900 560" role="img" aria-label="${safeLabel}">
      <defs>
        <linearGradient id="bg" x1="0" x2="1" y1="0" y2="1">
          <stop offset="0" stop-color="hsl(${hue}, 82%, 42%)"/>
          <stop offset="1" stop-color="hsl(${(hue + 45) % 360}, 86%, 28%)"/>
        </linearGradient>
      </defs>
      <rect width="900" height="560" fill="url(#bg)"/>
      <circle cx="720" cy="115" r="155" fill="rgba(255,255,255,.16)"/>
      <circle cx="170" cy="450" r="120" fill="rgba(255,255,255,.12)"/>
      <path d="M90 380 C230 250 330 470 480 305 C600 170 695 310 815 180" fill="none" stroke="rgba(255,255,255,.58)" stroke-width="28" stroke-linecap="round"/>
      <text x="70" y="115" fill="white" font-family="Arial, Helvetica, sans-serif" font-size="42" font-weight="800" letter-spacing="7">SRC SPORTS</text>
      <text x="70" y="490" fill="white" font-family="Arial, Helvetica, sans-serif" font-size="68" font-weight="900">${safeLabel}</text>
    </svg>`;
  return `data:image/svg+xml;charset=UTF-8,${encodeURIComponent(svg)}`;
}

function pickVisualForCard(card, usedVisuals) {
  const text = `${card.dataset.search || ""} ${card.textContent || ""}`.toLowerCase();
  const directMatch = SPORT_VISUALS.find((visual) => visual.keywords.some((keyword) => text.includes(keyword)));

  if (directMatch && !usedVisuals.has(directMatch.key)) {
    return directMatch;
  }

  const unusedMatch = SPORT_VISUALS.find(
    (visual) => !usedVisuals.has(visual.key) && visual.keywords.some((keyword) => text.includes(keyword))
  );
  ...
  return SPORT_VISUALS.find((visual) => !usedVisuals.has(visual.key)) || directMatch || SPORT_VISUALS[0];
}
...
function enrichVisualCards() {
  ...
  document.querySelectorAll(".sport-card, .program-card, .event-card, .blog-card, .news-card, .content-card, .mini-panel").forEach((card) => {
    if (card.querySelector(".card-media")) {
    ...
    const visual = pickVisualForCard(card, usedVisuals);
    usedVisuals.add(visual.key);
...
function addSiteBackgroundVideo() {
  ...
  video.poster = BACKGROUND_VIDEO.poster;
  video.innerHTML = `<source src="${BACKGROUND_VIDEO.src}" type="video/mp4">`;
  ...
  document.body.insertBefore(video, scrim);
}
